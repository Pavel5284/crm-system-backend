...
import { NOTIFICATION_EVENTS, TaskAssignedEventPayload, TaskCompletedEventPayload } from '@app/shared';
...
export class NotificationsService {
    ...
    async handleTaskAssigned({ task, actorId }: TaskAssignedEventPayload) {
        ...
        await this.notify(task.assigneeId, NotificationType.TASK_ASSIGNED, { taskId: task.id, title: task.title });
    }

    async handleTaskCompleted({ task, actorId }: TaskCompletedEventPayload) {
        if (task.authorId === actorId) return;
        await this.notify(task.authorId, NotificationType.TASK_COMPLETED, { taskId: task.id, title: task.title });
    }
