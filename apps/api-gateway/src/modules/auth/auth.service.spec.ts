...
describe('AuthService', () => {
    ...
    beforeEach(async () => {
        ...
        configService = { getOrThrow: jest.fn((key: string) => `value-for-${key}`) };
    ...
    it('бросает ConflictException при регистрации с занятым email', async () => {
        ...
        await expect(
            service.register({ email: 'a@a.com', password: 'password123', name: 'A' }),
    ...
    it('успешный логин возвращает пару токенов', async () => {
        prisma.user.findUnique.mockResolvedValue({
            id: '1', email: 'a@a.com', role: 'USER', passwordHash: 'hash',
        });
